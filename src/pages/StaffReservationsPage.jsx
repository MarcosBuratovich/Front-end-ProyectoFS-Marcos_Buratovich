...
const StaffReservationsPage = () => {
  ...
  useEffect(() => {
    ...
    const isAuthorized = user && (["staff", "admin"].includes(user.role) || ["admin", "staff"].includes(user.username))
  ...
  const handleMarkPaid = async (reservationId) => {
    if (window.confirm("¿Confirmas que esta reserva ha sido pagada?")) {
      try {
        ...
        const updatedReservations = reservations.map((res) => (res._id === reservationId ? result.reservation : res))
  ...
  const handleStormRefund = async (reservationId) => {
    if (window.confirm("¿Confirmas que la reserva se vio afectada por condiciones climáticas y se debe reembolsar el 50%?")) {
      try {
        ...
        const updatedReservations = reservations.map((res) => (res._id === reservationId ? { ...res, paymentStatus: "partial refund", cancellationStatus: "storm refund", refundAmount: result.refundAmount || res.totalPrice * 0.5 } : res))
        setReservations(updatedReservations)
      } catch (err) {
  ...
  const handleCancelReservation = async (reservationId) => {
    if (window.confirm("¿Estás seguro de que quieres cancelar esta reserva?")) {
      try {
        ...
        const updatedReservations = reservations.map((res) => (res._id === reservationId ? { ...res, cancellationStatus: "canceled", paymentStatus: "canceled" } : res))
        setReservations(updatedReservations)
      } catch (err) {
  ...
  const getCancellationStatusText = (status) => {
    switch (status) {
      ...
      case "payment-expired":
        return "Expirada por falta de pago"
      case "canceled":
        return "Cancelada por usuario"
      case "storm refund":
        return "Reembolso por tormenta"
      default:
        return status
    }
  }
      ...
      <div className="dashboard-card">
        <div className="filters-container">
          ...
          <div className="filter-group">
            ...
            {dateFilter && (
              <button onClick={() => setDateFilter("")} className="clear-filter-btn">
                ×
              </button>
            )}
          </div>
                  ...
                  <React.Fragment key={res._id}>
                    <tr className={`reservation-row ${res.paymentStatus === "canceled" ? "canceled" : ""}`}>
                      ...
                      <td className={getPaymentDeadlineStatus(res.paymentDeadline)}>
                        {res.paymentDeadline
                          ? new Date(res.paymentDeadline).toLocaleTimeString("es-ES", {
                              hour: "2-digit",
                              minute: "2-digit",
                              day: "2-digit",
                              month: "2-digit",
                      ...
                      <td>
                        <span className={`status status-${res.paymentStatus}`}>{getPaymentStatusText(res.paymentStatus)}</span>
                      </td>
                      <td>
                        <div className="action-buttons">
                          ...
                          {res.paymentStatus === "pending" && (
                            <>
                              <button onClick={() => handleMarkPaid(res._id)} className="action-button pay-button" title="Marcar como pagada">
                                💰 Pagada
                              </button>
                              <button onClick={() => handleCancelReservation(res._id)} className="action-button cancel-button" title="Cancelar esta reserva">
                                ❌ Cancelar
                              </button>
                            </>
                          )}
                          ...
                          {res.paymentStatus === "paid" && (
                            <>
                              <button className="action-button refund-button" title="Aplicar seguro de tormenta (50% de devolución)" onClick={() => handleStormRefund(res._id)}>
                                🌧️ Reembolso 50%
                              </button>
                              <button className="action-button receipt-button" title="Ver recibo" onClick={() => alert("Funcionalidad de recibo en desarrollo")}>
                                🧾 Recibo
                              </button>
                            </>
                          )}

                          <button onClick={() => toggleExpandReservation(res._id)} className="action-button details-button" title="Ver detalles completos">
                          ...
                          <div className="details-container">
                            ...
                            <div className="details-grid">
                              <div>
                                ...
                                <p>
                                  <strong>Nombre:</strong> {res.customer?.name || "N/A"}
                                </p>
                                <p>
                                  <strong>Contacto:</strong> {res.customer?.contact || "N/A"}
                                </p>
                              </div>
                              ...
                              <div>
                                ...
                                {res.discount > 0 && (
                                  <p>
                                    <strong>Descuento aplicado:</strong> ${res.discount}
                                  </p>
                                )}
                                <p>
                                  <strong>Total:</strong> ${res.totalPrice}
                                </p>
                                {res.refundAmount > 0 && (
                                  <p>
                                    <strong>Reembolso aplicado:</strong> ${res.refundAmount}
                                  </p>
                                )}
                              </div>
                              ...
                              <div>
                                ...
                                <p>
                                  <strong>Estado de pago:</strong> {getPaymentStatusText(res.paymentStatus)}
                                </p>
                                <p>
                                  <strong>Estado de cancelación:</strong> {getCancellationStatusText(res.cancellationStatus)}
                                </p>
                                <p>
                                  <strong>Fecha reservada:</strong> {formatDate(res.date)}
                                </p>
                                <p>
                                  <strong>Horas:</strong> {formatSlots(res.slots)}
                                </p>
                                <p>
                                  <strong>Deadline de pago:</strong> {res.paymentDeadline ? new Date(res.paymentDeadline).toLocaleString() : "N/A"}
                                </p>
                              </div>
                            </div>
                          </div>
